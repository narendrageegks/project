...
def temp1(request):
    ...
    if request.session.keys():
        return render(request, 'temp1.html', {'condition': True,'resumeheader':resumeheader,'educate':educate,'jobs':jobs,'intern':intern,'train':train,'project':project,'skill':skill,'add':add})
    else:
        return render(request, 'index.html', {'condition': False})
